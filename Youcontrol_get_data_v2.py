...
def get_headers():
    """Генерує випадкові хедери для запиту."""
    return {
...
def parse_all_kved():
    ...
    for section_block in section_blocks:
        ...
        for chapter_td in chapter_tds:
            ...
            logger.debug(f"Found {len(rows)} rows in chapter {chapter_code}")
            ...
            for row in rows:
                ...
                if class_code_td and current_group_code:
                    ...
                    # Save batch after processing the class
                    if batch_data:
                        data.extend(batch_data)
                        df_batch = pd.DataFrame(batch_data)
                        file_path = "youcontrol_full_dataset.csv"
                        if os.path.exists(file_path):
                            df_batch.to_csv(file_path, mode='a', header=False, index=False)
                        else:
                            df_batch.to_csv(file_path, mode='w', header=True, index=False)
                        logger.info(f"Saved batch for class {class_code} to {file_path}")
                        time.sleep(30)

    logger.info(f"Completed parsing. Total rows processed: {len(data)}")
    return pd.DataFrame(data)

if __name__ == "__main__":
    logger.info("Starting main execution")
    try:
        df_all = parse_all_kved()
        logger.info("Data saved incrementally to youcontrol_full_dataset.csv")
        print(df_all.head())
    except Exception as e:
